...
if exist "%~dp0tools\NekoLogSimulator.exe" (
    "%~dp0tools\NekoLogSimulator.exe" --dir "%~dp0sample_logs" --static --events 5 > nul 2>&1
    echo 2. Launching background live log streamer...
    start "PSO2 Live Log Streamer" cmd /k "title PSO2 NGS Log Streamer && "%~dp0tools\NekoLogSimulator.exe" --dir "%~dp0sample_logs" --stream --interval 2.5"
) else (
