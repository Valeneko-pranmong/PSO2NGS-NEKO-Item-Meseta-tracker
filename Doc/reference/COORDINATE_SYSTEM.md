...
## 6. การฟาร์มต่อเนื่องหลายช่องย่อย (Multi-Slot Continuous Meseta Accumulation)

เพื่อให้สอดคล้องกับปรัชญา **"เกมแค่เติมเงินเข้าไปในช่อง ใครใส่เยอะคนนั้นเป็นเจ้าของ"** ระบบอนุญาตให้ผู้เล่นย้ายพิกัดและช่องย่อยเพื่อฟาร์มเงินสะสมไปเรื่อยๆ ทั่วทั้งกาแล็กซี:

1. **โครงสร้างจัดเก็บเงินรายช่อง (`slot_farmed`):**
   - ใน `WarService` จัดเก็บยอดเงินแยกตามคีย์ `f"{coord_key}#{slot}"` (เช่น `"0,0#1": 10000000`, `"3,3#2": 4500000`)
   - เมื่อผู้เล่นได้รับ Meseta เงินจะถูกบวกเพิ่มเข้าทั้งช่องย่อยปัจจุบัน (`slot_farmed[key]`), ยอดรวมของเซสชัน (`session_contribution`), และยอดรวมตลอดกาล (`total_farmed`)
2. **การคงอยู่ของยอดเงินเมื่อสลับพิกัด (Zero Relocation Wiping):**
   - เมื่อผู้เล่นเปลี่ยนพิกัดเป้าหมายหรือสลับช่องย่อย เงินในช่องย่อยเดิมที่เคยฟาร์มไว้ **จะไม่ถูกย้ายตาม และจะไม่ถูกส่งคำสั่ง `departed` ล้างเป็น 0**
   - ผู้เล่นสามารถกลับมาฟาร์มต่อในช่องเดิมได้ตลอดเวลา หรือกระจายเงินไปยึดครองช่องย่อยอื่นๆ ได้อย่างอิสระ
3. **การคำนวณยอดเงินรวมระดับ Sector:**
   - ฟังก์ชัน `get_sector_meseta(coord_key)` คำนวณผลรวมเงินสะสมจากทั้ง 4 ช่องย่อยใน Sector นั้นๆ:
     $$\text{Sector Meseta} = \sum_{s=1}^{4} \text{slot\_farmed}\left[f"{coord\_key}\#{s}"\right]$$
4. **พิกัดเริ่มต้นที่เป็นกลาง (Default Core Coordinate):**
   - เมื่อเปิดแอปพลิเคชันทุกครั้ง ระบบจะกำหนดพิกัดเริ่มต้นไปที่ **Core `[0, 0, 1]`** โดยอัตโนมัติ ไม่จำพิกัดเก่าจากรอบก่อน เพื่อให้ผู้เล่นตัดสินใจเลือกแนวรบใหม่ในแต่ละเซสชันอย่างเป็นอิสระ
